...
| Approximate run-history size next to the retention setting | Marked "implement now" in the assessment (§15) but did not ship in 0.9.0 and was recorded in neither ledger — surfaced by a later docs audit | Reuse of the Settings storage-size walk already added for Network & storage | 0.10 |
